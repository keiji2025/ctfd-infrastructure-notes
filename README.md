...
2. KIT OSINT CTF  
   学内CTFイベントの開催に向けて、部室のオンプレミスサーバー上にCTFdを構築・運用しました。既存サービスとの共存、学内ネットワーク制約、DockerネットワークのIP帯重複、運用時の安定性を考慮しました。
...
構築作業だけでなく、各設定の意味を理解し直す時間も含めて取り組みました。

## 目的

クラウドインフラ、ネットワーク、セキュリティの基礎を実践的に学ぶため、個人学習としてAzure上にCTFプラットフォームを構築しました。

単にCTFdを動かすだけではなく、クラウド上でサービスを公開する際に必要となる以下の要素を一連の流れとして経験することを目的としました。
...
Azure for Students環境の制約により、デプロイ可能なリージョンが制限されていた点で詰まりました。

Azureポータル上では検証に成功していても、実際のデプロイ時にAzure Policyによって弾かれることがありました。

そのため、アクティビティログやJSON形式のエラーを確認し、利用可能なリージョンを特定しました。

この経験から、クラウド環境ではポータル上の入力が正しく見えても、サブスクリプションやポリシーによる制約でデプロイに失敗する場合があることを学びました。

### SSH秘密鍵の権限エラー

SSH接続時には、秘密鍵ファイルの権限が広すぎるため、`WARNING: UNPROTECTED PRIVATE KEY FILE!` というエラーが発生しました。

Windows上のファイル権限や保存場所を見直すことで、秘密鍵が適切に保護されていないとSSH接続が拒否されることを学びました。

### Docker / Nginx / HTTPS化まわりの理解

Docker導入時のGPGキーや公式リポジトリ設定、Let’s Encrypt証明書をNginxコンテナに読み込ませるためのdocker-compose.yml修正、NginxのHTTPS設定などは、最初は手順を追う形になりました。
